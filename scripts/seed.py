...
async def seed_data():
    ...
    async with AsyncSessionLocal() as session:
        ...
        # 4.5 Seed Notebooks
        notebook_items = [
            ("AI Engineering", "🤖", "Artificial Intelligence, LLMs, Agents and RAG architectures"),
            ("Databases", "🗄", "SQL, PostgreSQL, pgvector, Vector Search and caching systems"),
            ("Backend", "💻", "Web APIs, FastAPI, Authentication, Microservices and Node.js"),
            ("Frontend", "🎨", "React, UI/UX design, CSS architecture and state management"),
        ]
        notebook_entities = {}
        for nb_name, nb_icon, nb_desc in notebook_items:
            nb = Notebook(
                user_id=user_id,
                name=nb_name,
                icon=nb_icon,
            ...
            session.add(nb)
            notebook_entities[nb_name] = nb
        await session.flush()
        print(f"✅ Created {len(notebook_items)} Notebooks")
        ...
        for title, content, summary, nb_name, note_tags, note_concepts in notes_data:
            ...
            note = Note(
                ...
                content=content,
                summary=summary,
                notebook_id=notebook_entities[nb_name].id if nb_name in notebook_entities else None,
