...
class NoteRepository:
    ...
    async def create(
        ...
        self.db.add(note)
        await self.db.flush()
        await self.db.refresh(note, ["tags", "concepts", "notebook"])
        return note
    ...
    async def list_by_user(
        ...
        if notebook_id and notebook_id.strip() and notebook_id != "all":
            stmt = stmt.where(Note.notebook_id == notebook_id.strip())
            count_stmt = count_stmt.where(Note.notebook_id == notebook_id.strip())
        if tag_id:
    ...
    async def update(
        ...
        if notebook_id is not None:
            note.notebook_id = notebook_id if notebook_id.strip() else None
